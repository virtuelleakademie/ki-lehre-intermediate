...
- **Vertiefung**: Rekonstruktives Gedächtnis & Schemas + Konsolidierung
...
1. **Gedächtnis ist kein Aufnahmegerät** --- es ist ein rekonstruktiver Prozess, der Informationen bei jeder Stufe transformiert
...
3. **Der Übergang von episodischem zu semantischem Wissen** erfordert aktive, variierende Begegnungen --- er geschieht nicht von allein
...
6. **Missverständnisse kann man nicht durch korrektes Präsentieren überschreiben** --- Schemas schreiben neue Information still um
...
**Episodisches Gedächtnis** speichert persönlich erlebte Ereignisse, gebunden an einen bestimmten Zeitpunkt und Ort. Wenn Studierende sich an *Ihre Vorlesung* erinnern --- den Raum, die Folie, das Beispiel --- ist das episodisches Gedächtnis. Es ist reichhaltig, kontextgebunden und inhärent an die Umstände der Enkodierung geknüpft.

**Semantisches Gedächtnis** speichert dekontextualisiertes Wissen --- Fakten, Konzepte, Prinzipien, Kategorien. Wenn Studierende wissen, dass *Mitochondrien die Kraftwerke der Zelle sind*, ohne sich zu erinnern, wo sie das gelernt haben --- das ist semantisches Gedächtnis. Es ist abstrakt, flexibel und (idealerweise) über Kontexte hinweg transferierbar.

### Prozedurales Gedächtnis: Wissen, wie

Prozedurales Gedächtnis speichert Fähigkeiten und Abläufe --- Dinge, die man *tun* kann, aber möglicherweise nicht erklären kann. Studierende, die Differentialgleichungen flüssig lösen, aber Mühe haben, die Regeln zu artikulieren, operieren aus dem prozeduralen Gedächtnis.
...
**Ohne zurückzuschauen:** Ordnen Sie diese Beispiele dem richtigen Gedächtnissystem zu:
...
Einer der wichtigsten Prozesse beim Lernen ist die Entwicklung von stark **episodischen** Erinnerungen zu **semantischem** Wissen --- vom Erinnern einer bestimmten Erfahrung zum Besitz abstrakten, transferierbaren Verständnisses.
...
- **Abstraktion** --- explizites Identifizieren der Tiefenstruktur über Beispiele hinweg
...
**Lehrimplikation**: Um transferierbares Wissen aufzubauen, müssen Sie bewusst die **Kontexte variieren**, in denen Studierende Konzepte begegnen und üben. Verschiedene Beispiele, verschiedene Oberflächenmerkmale, verschiedene Aufgabenformate verwenden. Jeder neue Kontext schafft zusätzliche Abrufpfade. Das ist auch ein starkes Argument für **Interleaving** --- das Mischen verschiedener Aufgabentypen statt ihrer Blockung --- weil Interleaving Studierende zwingt, die *Auswahl* des richtigen Wissens zu üben, nicht nur dessen Ausführung.

::: {.reflect}
### Anwendung: Transfer in Ihrer Lehre

Denken Sie an eine Prüfungsfrage, bei der viele Studierende überraschend schlecht abschneiden --- obwohl das Material "behandelt" wurde:
...
- Was könnten Sie ändern, um mehr Transferpfade zu schaffen?
:::


## Gedächtnis ist rekonstruktiv

Vielleicht der kontraintuitivste Befund der Gedächtniswissenschaft [@schacterAdaptiveConstructiveProcesses2012]: **Gedächtnis ist nicht reproduktiv, es ist rekonstruktiv.** Wenn man sich an etwas erinnert, spielt man keine Aufnahme ab --- man rekonstruiert eine Erfahrung aus gespeicherten Fragmenten, ergänzt durch Vorwissen, aktuelle Überzeugungen und kontextuelle Hinweise.

### Schemas: Das zweischneidige Schwert

Schemas --- organisierte Wissensstrukturen, die aus Erfahrung aufgebaut werden --- sind gleichzeitig der grösste Vorteil des Gedächtnisses und seine systematischste Fehlerquelle.
...
**Lehrimplikation**: Studierende werden neue Information systematisch **an ihre bestehenden Schemas anpassen**. Das ist keine Nachlässigkeit --- so funktioniert Gedächtnis. Einfach korrekte Information zu *präsentieren* reicht oft nicht; Sie müssen das Missverständnis **explizit aktivieren, kognitiven Konflikt erzeugen und dann auflösen**.
...
1. **Aktivieren**: Machen Sie das Missverständnis explizit. Bitten Sie Studierende, sich auf eine Vorhersage festzulegen.
2. **Konflikt**: Zeigen Sie Evidenz, die das Missverständnis nicht erklären kann.
3. **Auflösen**: Präsentieren Sie die korrekte Erklärung im direkten Kontrast zum Missverständnis.

::: {.reflect}
### Missverständnisse in Ihrem Fach

Nennen Sie ein häufiges Missverständnis in Ihrem Fachgebiet:

- Wie äussert es sich bei Studierenden?
- Haben Sie bisher versucht, es durch Präsentation der korrekten Information zu korrigieren?
- Wie könnten Sie stattdessen die Activate-Conflict-Resolve-Strategie einsetzen?
:::


## Konsolidierung: Gedächtnis verändert sich nach der Enkodierung

Enkodierung ist nicht das Ende der Geschichte. Nach dem initialen Lernen durchlaufen Erinnerungen einen langwierigen Prozess der **Konsolidierung** --- sie werden über Stunden, Tage und Wochen schrittweise reorganisiert, verstärkt und mit bestehendem Wissen integriert.

### Zentrale Konsolidierungsprinzipien

**Schlaf konsolidiert Erinnerungen.** Im Schlaf werden kürzlich enkodierte Erinnerungen reaktiviert und mit bestehenden Wissensstrukturen integriert. Schlaf scheint bevorzugt den *Kern* des gelernten Materials zu konsolidieren --- die zugrundeliegenden Muster und Regeln --- eher als Oberflächendetails. (Dies ist stärker belegt für neutrales deklaratives Material; Schlaf kann unter bestimmten Bedingungen, besonders bei emotionalem Material, auch spezifische Details konsolidieren.) Lernen, das über mehrere Tage verteilt wird (mit Schlaf zwischen den Sitzungen), produziert qualitativ anderes Gedächtnis als dieselbe Lernmenge in einer einzigen Sitzung.
...
Hunderte von Studien haben gezeigt: Die Handlung des Abrufens --- eine Frage beantworten, eine Antwort generieren, ein Problem aus dem Gedächtnis lösen --- verstärkt die Gedächtnisspur wirksamer als erneutes Studieren desselben Materials.
...
- **Abruf aktualisiert den Kontext.** Jedes Abrufereignis enkodiert einen neuen Kontext, der mit der Erinnerung assoziiert wird --- macht sie aus mehr Situationen abrufbar (adressiert das Transfer-Problem).
...
2. Nennen Sie drei Gründe, warum Abrufpraxis funktioniert.
3. Muss Abrufpraxis benotet werden, um wirksam zu sein?

*Vergleichen Sie anschliessend. Wo lagen Sie richtig?*
:::
...
Lerneinheiten über die Zeit verteilen statt zusammenpressen. Derselbe Zeitaufwand --- dramatisch unterschiedliche Ergebnisse.

**Warum es funktioniert:** Jede verteilte Sitzung beinhaltet partielles Vergessen, gefolgt von anstrengungsreichem Abruf, der die Erinnerung verstärkt. Massed Learning erlaubt Aufrechterhaltung im Arbeitsgedächtnis ohne Abruf aus dem Langzeitgedächtnis --- fühlt sich leicht an, umgeht aber die Mechanismen, die dauerhafte Enkodierung erzeugen. Zusätzlich ermöglicht Spacing Konsolidierung zwischen den Sitzungen.
...
::: {.reflect}
### Anwendung: Wünschenswerte Schwierigkeiten in Ihrem Kurs

Analysieren Sie einen Ihrer Kurse:

1. Wo setzen Studierende hauptsächlich **Wiederlesen** ein? Wie könnten Sie **Abrufpraxis** einbauen?
2. Üben Studierende Aufgabentypen **blockweise**? Könnten Sie **Interleaving** einführen?
3. Gibt es Stellen, an denen Studierende **Antworten empfangen**, bevor sie selbst versucht haben? Wie könnten Sie den **Generierungseffekt** nutzen?

Identifizieren Sie eine konkrete Änderung, die Sie umsetzen könnten.
:::
...
Investieren Sie **15 Minuten** in der ersten Sitzung, um die Performance-Learning-Unterscheidung zu lehren. Zeigen Sie die Testing-Effect-Daten. Vermitteln Sie Abruf + Spacing + Interleaving. Diese einzelne Intervention verbessert das Lernen in **jedem Kurs**, den die Studierenden besuchen.
...
- Das **Evaluationsparadox** (→ [Teil 2](../../02-ki-und-lernen/)): Schemas, die KI-Output still umschreiben, sind derselbe Mechanismus wie Schemas, die Vorlesungsinhalte umschreiben
...
- Das **rekonstruktive Gedächtnis** erklärt, warum Cargo Cult Scholarship (→ [Teil 2](../../02-ki-und-lernen/)) so tückisch ist: Studierende *glauben*, sie hätten nachgedacht, weil ihr Schema die Rekonstruktion plausibel erscheinen lässt
