...
Diese Seite vertieft die wissenschaftlichen Grundlagen des Workshops. Empfohlene Reihenfolge:

- **Essentiell**: Die Progression von schwachen zu starken Methoden & Cognitive Load Theory
- **Vertiefung**: Vorhersagefehler-Mechanismus --- ergänzt die Workshop-Inhalte mit wissenschaftlicher Tiefe
- **Optional**: Grokking und Gedächtnisparadoxon --- interessante Erweiterungen

Abschnitte, die mit dem Workshop-Hauptinhalt überlappen, sind als *Vertiefung* markiert und bieten zusätzliche Details und Beispiele.
:::

## Zentrale Erkenntnisse

1. **Expertise erfordert schrittweisen Kompetenzaufbau** durch schwache → starke Methoden
2. **Kognitive Anstrengung ist kein Fehler, sondern die Funktion**, die Lernen durch Vorhersagefehler, Elaboration und Konsolidierung ermöglicht
...
::: {.reflect}
### Reflexion: Expertise in Ihrer Lehre

Denken Sie an ein Thema, das Sie Ihren Studierenden beibringen:

- Welche "schwachen Methoden" verwenden Anfänger typischerweise?
- Welche automatisierten Prozeduren haben Experten entwickelt?
- Was würde passieren, wenn Sie Anfängern direkt Experten-Werkzeuge geben?

*Notieren Sie 2--3 Beobachtungen aus Ihrer eigenen Lehre.*
:::
...
::: {.reflect}
### Reflexion: Cognitive Load erkennen

**Ohne zurückzuschauen**: Erklären Sie in eigenen Worten:

- Was ist der Unterschied zwischen extrinsischer und germane load?
- Warum können Experten mehr Elemente gleichzeitig verarbeiten?
- Wie beeinflusst das Ihre Aufgabengestaltung?

*Der Versuch, dies ohne Nachschlagen zu erklären, ist selbst ein Beispiel für Abrufpraxis.*
:::
...
Wenn Anfänger zu früh Expertenwerkzeuge nutzen, können sich mehrere Risiken verstärken:
...
6. **Das Selbstbild aktualisiert sich**: Lernende schliessen aus ihren Erfahrungen (mit KI: Erfolg, ohne KI: Misserfolg) rational, dass sie es allein nicht können

Diese Risiken bilden kein deterministisches Stufenmodell --- sie interagieren dynamisch und variieren je nach Person und Kontext. Das Ergebnis ist nicht "Verlernen" im engeren Sinne (aktiver Verlust von Wissen), sondern **Fähigkeitsatrophie durch Nichtgebrauch**: Vorhandene Kompetenzen werden nicht weiter gestärkt und verblassen über die Zeit.
:::

::: {.callout-note}
### Verbindung zum Workshop

Diese vier Prinzipien bilden die Grundlage für:

- Das **5-Fragen-Framework** in Teil 3 (Fragen 2 & 5 behandeln den Kompetenzaufbau und die Versuch-dann-Prüfe-Struktur)
- Die **Agent-Design-Aktivität** in Teil 3 (Leitfrage: "Was tut der Agent? Was tut der Lernende?")

Sie werden diese Prinzipien in der Praxis anwenden, wenn Sie Ihre eigene Aufgabe analysieren.
:::
...
Der folgende Abschnitt vertieft das Konzept des "Prediction Error Learning", das im Workshop eingeführt wurde. Hier sehen Sie die wissenschaftlichen Details und Mechanismen hinter diesem Prinzip.
:::

### Vorhersagefehler: Ein wichtiger Lernmechanismus

Prediction Error Learning ist einer der wichtigsten neuronalen Mechanismen für den Kompetenzaufbau. Er erklärt, warum bestimmte didaktische Strategien (wie Abrufpraxis und Generierung) wirksam sind. Die Forschung zu Vorhersagefehlern umfasst verschiedene Systeme --- dopaminerge Belohnungssignale, sensorische Vorhersagefehler und Überraschung im Bayesianischen Sinne --- die über unterschiedliche neuronale Substrate und Rechenprinzipien operieren. Für die Lehrpraxis ist die gemeinsame Grundidee entscheidend: *Das Gehirn lernt aus der Diskrepanz zwischen Erwartung und Erfahrung.* Daneben spielen Konsolidierung, Elaboration und Mustererkennung ebenfalls zentrale Rollen.
...
- **Synaptische Verbindungen** werden je nach Fehlergröße angepasst
...
::: {.reflect}
### Reflexion: Vorhersagefehler in Ihrer Lehre

Denken Sie an eine Aufgabe in Ihrer Lehre:

- Wo generieren Studierende Vorhersagen, bevor sie Feedback bekommen?
- Wo könnte KI diesen Vorhersagefehler-Zyklus kurzschließen?
- Wie könnten Sie die "Versuch-dann-Prüfe"-Struktur einbauen?
:::
...
Das entscheidende Problem: Je weniger Vorwissen Lernende haben, desto weniger können sie mit dem KI-Output anfangen. Ihr internes Modell ist noch zu undifferenziert, um dem Feedback Bedeutung zuzuschreiben --- sie erkennen weder, *warum* die Antwort richtig ist, noch *wo* ihr eigenes Verständnis die Lücken hat.
...
Das erklärt, warum Lernende hunderte Beispiele anschauen oder tausende korrekte KI-Antworten bekommen und trotzdem nicht lernen. **Lernen braucht informative Fehler, nicht Perfektion.**

### Das interne Modell: Was Expertise eigentlich ist

Ein Experte "weiss" nicht einfach mehr Fakten. Expertise bedeutet, ein **internes generatives Modell** aufgebaut zu haben --- ein Modell, das eigenständig Vorhersagen, Erklärungen und Lösungen produzieren kann. Dieses Modell ist es, was sich durch tausende Übungszyklen entwickelt.

Warum ist diese Perspektive hilfreich? Sie erklärt auf einen Blick:
...
::: {.pro-tip collapse="true"}
### Vertiefung: Die Bayesianische Perspektive (optional)

Man kann den Vorhersagefehler-Mechanismus formal als Bayesianisches Lernen beschreiben. Ein Lernender hat ein internes Modell $M$ mit Überzeugungen $\theta$ und aktualisiert diese durch neue Daten $d$:

$$P(\theta|d, M) \propto P(d|\theta, M) \cdot P(\theta|M)$$

Das heisst: Die neuen Überzeugungen (Posterior) ergeben sich aus dem Zusammenspiel von Vorwissen (Prior) und der Frage, wie gut die neuen Daten zu verschiedenen Hypothesen passen (Likelihood).

**Warum das für KI relevant ist:** Wenn ein Novize einen KI-Output liest, ist seine Likelihood-Funktion $P(d|\theta)$ *flach* --- korrekte und inkorrekte Antworten sehen für ihn ähnlich plausibel aus. Der Informationsgewinn ist minimal. Ein Experte hingegen hat eine *scharfe* Likelihood: Er kann sofort erkennen, ob der Output zu seinem Modell passt oder nicht.

Lernen im Bayesianischen Sinne ist **Entropiereduktion** --- die Reduktion von Unsicherheit über die Struktur der Welt. Aktives Problemlösen maximiert den erwarteten Informationsgewinn, weil es die Lernenden zwingt, ihre eigenen Vorhersagen zu testen. KI-Outsourcing minimiert ihn, weil die Lernenden nie ihre eigenen Vorhersagen generieren.
:::
...
- **Verminderte Problemlösefähigkeiten** bei Mathematiklernenden mit KI-Tools [@bastaniGenerativeAIGuardrails2025]
- **„Die Fluency-Illusion"**: KI-Kompetenz wird mit eigenem Können verwechselt

::: {.reflect}
### Reflexion: Das Gesamtbild

Bevor Sie weiterlesen, fassen Sie für sich zusammen:

- Was sind die drei wichtigsten Erkenntnisse für Ihre Lehre?
- Welche **eine Änderung** an einer Aufgabe werden Sie zuerst umsetzen?
- Wo erwarten Sie den größten Widerstand von Studierenden?
...
::: {.pro-tip collapse="true"}
### Optionale Vertiefung: Das "Grokking"-Phänomen
...
Grokking (im maschinellen Lernen) zeigt:

- Nach perfektem Memorieren kann echtes Verstehen entstehen
- Manchmal braucht es viel mehr Trainingsschritte als bis zum Memorieren
- Das Modell findet plötzlich die zugrundeliegende Regel statt nur Beispiele auswendig zu lernen
:::

::: {.pro-tip collapse="true"}
### Optionale Vertiefung: Das Gedächtnisparadoxon (Flynn-Effekt)
...
- Nur uninformative Vorhersagefehler (kein eigener Versuch vor KI-Nutzung)
:::
